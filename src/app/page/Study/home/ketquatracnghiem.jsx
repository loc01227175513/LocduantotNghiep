...
export default function KetQuaTracNghiem({ idBaihoc, idTracNghiem }) {
    ...
    //   console.log(dapAnNguoiDung, "dapAnNguoiDung");
    //   console.log(checkToanBocauhoi, "checkToanBocauhoi");

    useEffect(() => {
        if (checkToanBocauhoi.length > 0 && dapAnNguoiDung.length > 0) {
            // Flatten checkToanBocauhoi if it's a nested array
            const flatCheckToanBocauhoi = checkToanBocauhoi.flat();
            console.log(flatCheckToanBocauhoi, "flatCheckToanBocauhoi");

            const flatDapAnNguoiDung = dapAnNguoiDung.flat();

            console.log(flatCheckToanBocauhoi, "flatCheckToanBocauhoi");
            console.log(flatDapAnNguoiDung, "flatDapAnNguoiDung");

            // Ensure there is at least one item after flattening
            ...
            const CheckIdBaiTracNghiem = flatDapAnNguoiDung.every((item) => {
                if (item.id_baitracnghiem !== undefined && item.id_baitracnghiem !== null) {
                    // console.log(item.id_baitracnghiem, "item.id_baitracnghiem");
            ...
            console.log(CheckIdBaiTracNghiem, "CheckIdBaiTracNghiem");

            if (CheckIdBaiTracNghiem) {

                const TongSoDapAnDung = [];
            
                // Iterate through each baiTracNghiem to collect all correct answers
                flatCheckToanBocauhoi.forEach((baiTracNghiem) => {
                    console.log(baiTracNghiem, "baiTracNghiem");
                    ...
                    // Accumulate all correct answers into TongSoDapAnDung
                    TongSoDapAnDung.push(...correctAnswers);
                });
            
                const sodapanDung = TongSoDapAnDung.length;
                console.log(TongSoDapAnDung, "TongSoDapAnDung");
            
                // Collect user's correct answers
                const SocaudapandungUser = flatDapAnNguoiDung
                    .map((item) => item.stt);
                console.log(SocaudapandungUser, "Dapan");
                ...
                // Compare the correct answers with user's answers
                const check = arraysEqual(TongSoDapAnDung, SocaudapandungUser);
                console.log(check ? "Answers match." : "Answers do not match.");
            
                if (check) {
                    // Send result or trigger next action when answers are correct
    ...
    if (!Array.isArray(questions) || questions.length === 0) {
        console.error("Không có câu hỏi trong bài trắc nghiệm:", {
            quizDetails,
            idTracNghiem,
        });
        return (
    ...
    const allQuestions = questions.flatMap((questionObj) => {
        if (
            !Array.isArray(questionObj.cau_hoi) ||
            !Array.isArray(questionObj.cau_traloi)
        ) {
            return [];
        }
