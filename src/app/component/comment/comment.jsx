                ...
                {displayedComments.map((item, index) => (
                    <div key={index} className="bg-white rounded-lg shadow-lg p-6 border border-gray-200 hover:shadow-xl transition-shadow duration-300">
                        <div className="flex items-center space-x-4">
                            ...
                            <div>
                                <h6 className="font-semibold text-lg">{item.nguoi_danh_gia.ten}</h6>
                                <div className="px-3 py-1 bg-teal-100 rounded-full text-teal-800 text-sm font-medium">
                                    Học viên
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-4">
                            ...
                            <button className="text-teal-600 hover:text-teal-800 text-sm font-medium mt-2 transition-colors duration-200">
                                Xem tiếp →
                            </button>
                        </div>
                        ...
                        <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                            <div className="flex items-center space-x-1">
                                <span className="font-medium">{item.so_sao}</span>
                                <div className="flex">
                                    {Array.from({ length: 5 }).map((_, idx) => (
                                        <svg key={idx} className={`w-5 h-5 ${idx < item.so_sao ? 'text-yellow-400' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
                                            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                                        </svg>
                                    ))}
                                </div>
                            </div>
                            <span className="text-sm text-gray-500">20/20/2020</span>
                        </div>
                    </div>
                ))}
            </div>
